...
export function setupRecordPicker() {
  ...
  async function search(more = false) {
    ...
    try {
      const response = await chrome.runtime.sendMessage({ type: 'odoo-metadata', operation: 'record-options', model: options.model, database: options.database, query: $('#record-picker-search').value.trim(), offset });
