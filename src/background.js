...
chrome.runtime.onMessage.addListener((message, sender, reply) => {
  ...
  (async () => {
    ...
    if (!['session', 'models', 'fields'].includes(message.operation)) throw new Error('Unsupported metadata request.');
    ...
    const request = { operation: message.operation, origin, database: String(message.database || ''), model: String(message.model || ''), query: String(message.query || '').slice(0, 100), offset: message.offset };
