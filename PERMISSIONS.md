...
- **activeTab:** temporary access to the tab where the user clicks the extension icon. This is used to query the selected Odoo installation's authenticated model/field metadata. No permanent website access is requested.
- **scripting:** runs a packaged, self-contained function in that tab's isolated world. Its only allowed operations are session verification, model listing through ir.model.search_read and fields_get metadata loading. It does not scrape the page or read/write business records.
...
The service worker accepts requests only from this extension's wizard page, verifies the source tab/origin and restricts operations to the metadata allowlist. Injected requests stay on that origin and reject redirects. Session/database mismatches and access errors stop metadata loading. The wizard's own CSP blocks direct network connections.

## Chrome Web Store disclosure guidance

- Remote code: **No**. Executable code is bundled with the extension.
- Developer or third-party data collection: **None**. Metadata is read directly from the user's selected Odoo server and used locally for the requested feature.
- Explain local data use accurately: drafts and selected origin/database/model are stored locally; temporary session and field metadata are processed to provide model-aware input.
